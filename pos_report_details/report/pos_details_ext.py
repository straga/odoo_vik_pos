...
def try_category(form_category ):
    res = False
    try:
        res = form_category
...
class pos_details_ext(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self, form):
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids):
            for pol in pos.lines:
                ...
                pol_category = pol.product_id.product_tmpl_id.pos_categ_id.id
                form_category = try_category(form['category_id'])

                if form_category:
                    if pol_category in form_category:
                        toresult = True
                    else:
                        toresult = False
                else:
    ...
    def _get_cat(self, form):

        self.cat = 'N/A'
        form_category = try_category(form['category_id'])

        if form_category:
            self.cat = form_category[1]

        return self.cat

    def _get_cat2(self, form):
    ...
    def _get_tax_amount(self, form):
        ...
        for order in pos_order_obj.browse(self.cr, self.uid, pos_ids):
            for line in order.lines:
                ...
                pol_category = line.product_id.product_tmpl_id.pos_categ_id.id
                form_category = try_category(form['category_id'])
    ...
    def __init__(self, cr, uid, name, context):
        super(pos_details_ext, self).__init__(cr, uid, name, context=context)
        ...
        self.localcontext.update({
            ...
            'get_cat' : self._get_cat,
            'get_cat2' : self._get_cat2,
        })


class report_pos_details(osv.AbstractModel):
    _name = 'report.pos_report_details.report_detailsofsales_ext'
    _inherit = 'report.abstract_report'
    _template = 'pos_report_details.report_detailsofsales_ext'
    _wrapped_report_class = pos_details_ext
